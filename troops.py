...
def get_image(troop, style):
    ...
    if style == "train":
        return object.train
    if style == "donate1":
        return object.donate1
    if style == "donate2":
        return object.donate2
    if style == "attack":
...
def get_super_troop(troop):
    ...
    if troop == super_minion:
        troop_image = i_super_minion
    images = [
        i_boost,
        i_boost_on,
        troop_image,
        ...
        i_000,
        i_000v2,
        i_red_cross_super_troops,
    ]
    for image in images:
        time.sleep(0.5)
        if image == i_super_minion:
            pag.moveTo(1070, 928)
            pag.dragTo(1070, 287, 0.6)
...
class Troop:
    def __init__(
        ...
        if type not in ["hero", "clan"]:
            ...
            if f"{self.name}_train.png" in train_files:
                self.i_train = Image(
                    name=f"i_{name}_train",
                    file=f"{train_directory}{name}_train.png",
                    no_of_regions=1,
                    type="train",
                )
            else:
                self.i_train = Image(
                    name=f"i_{name}_train",
                    file=f"{troop_directory}{name}_train.png",
                    no_of_regions=1,
                    type="train",
                )

            self.i_army = Image(
                name=f"i_{name}_army",
                file=f"{troop_directory}{name}_army.png",
                no_of_regions=1,
                type="army",
                threshold=0.72,
            )
            self.i_training = Image(
                name=f"i_{name}_training",
                file=f"{troop_directory}{name}_training.png",
                no_of_regions=1,
                type="training",
            )
            self.i_donate1 = Image(
                name=f"i_{name}_donate1",
                file=f"{troop_directory}{name}_donate1.png",
                no_of_regions=1,
                type="donate1",
            )
            self.i_donate2 = Image(
                name=f"i_{name}_donate2",
                file=f"{troop_directory}{name}_donate2.png",
                no_of_regions=2,
                type="donate2",
            )
            self.i_castle = Image(
                name=f"i_{name}_castle",
                file=f"{troop_directory}{name}_castle.png",
                no_of_regions=1,
                type="castle",
            )
            if "super" not in name:
                self.i_research = Image(
                    name=f"i_{name}_research",
                    file=f"{troop_directory}{name}_research.png",
                    no_of_regions=1,
                    type="research",
                )
        else:
        ...
        self.i_attack = Image(
            name=f"i_{name}_attack", file=f"{troop_directory}{name}_attack.png"
        )
        ...
        troops.append(self)
        if type == "troop":
            just_troops.append(self)
        if type == "spell":
            spells.append(self)
        if type == "siege":
            siege_troops.append(self)
    ...
    def in_castle(self):
        val, loc, rect = find(
            self.i_army.image, get_screenshot(CASTLE_TROOPS), show_image=False
        )
        return val > 0.8
    ...
    def delete(self, count):
        # print("Troop delete")
        if self.i_army.image is None:
            return
        goto(army_tab)
        i_army_edit.click()
        val, loc, rect = find(self.i_army.image, get_screenshot(ARMY_SPELLS_EXISTING))
        rect_adj = [
            rect[0] + ARMY_EXISTING[0],
            rect[1] + ARMY_EXISTING[1],
            rect[2],
            rect[3],
        ]
...
def load_troops():
    ...
    for row in range(2, sheet.max_row + 1):
        ...
        Troop(
            ...
            type=type,
            slide=slide,
            castle_slide=castle_slide,
            training_time=training_time,
            donate_bool=donate_bool,
            donate_preference=donate_preference,
            donations=donations,
...
def check_troop_colour_train(troop):
    ...
    rect_adj = [
        rect[0] + TRAIN_RANGE[0],
        rect[1] + TRAIN_RANGE[1],
        rect[2],
        rect[3],
    ]
