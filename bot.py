...
def get_resources(village):
    if village == "main":
        images = resource_images_main
    else:
        images = resource_images_builder
    zoom_out()
    for x in images:
        x.click()
...
def get_trader_info(account):
    ...
    if i_trader.click():
        i_raid_medals.click()
        pag.moveTo(1000, 900)
        pag.dragTo(1000, 280, 0.2)
        time.sleep(0.3)
        get_screenshot(
            CLOCK_POTION, filename=f"tracker/trader_clock_potion{account.number}"
        )
        get_screenshot(
            RESEARCH_POTION, filename=f"tracker/trader_research_potion{account.number}"
        )
        pag.press("space")
...
def sweep_full(fast=False):
    ...
    for account in accounts:
        ...
        #     time.sleep(0.1)
        get_screenshot(
            REMAINING_ATTACKS, filename=f"tracker/remaining_attacks{account.number}"
        )
        goto(main)
        if account.th > 5:
            coin()
...
def run_job(job):
    ...
    if time_to_string(job_time) == "Now":
        ...
        if admin.inviting and account.number <= 3:
            ...
            try:
                no_of_loops = min(49 - admin.no_of_members, 5)
            except:
                no_of_loops = 1
            if no_of_loops >= 1:
        ...
        for village in ["main", "builder"]:
            ...
            for account_r, village_r, currency_r, building_r, cost, comment in output:
                if account == account_r and village == village_r:
                    ...
                    if currency_r == "dark":
                        building[1] = building_r
                    if currency_r == "gold":
                        building[2] = building_r
                    if currency_r == "elixir":
                        building[3] = building_r
            spacer = ""
            if village == "main":
                spacer = "   "
...
def update_time_build(account, village):
    ...
    print(
        "Get time build: ", account_r, village_r, currency_r, building_r, cost, comment
    )
    time_temp = string_to_time(comment)
    if time_temp > datetime.now():
        return time_temp
